...
// Keep CLI parsing separate from `main` so tests can exercise the library.
#[derive(Parser)]
#[command(name = "resip")]
#[command(version)]
...
fn test() -> Result<()> {
    ...
    // This checks the full chain: Clash must accept the local proxy request,
    // then send it through the SSH tunnel to the remote proxy.
